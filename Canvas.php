...
class Canvas
{
    ...
    private function hex_to_rgb($hex_color)
    {
        ...
        $this->rgb = [hexdec(substr($hex_color, 0, 2)),
    ...
    {
        $this->crop_coordinates = [$x, $y, $this->width, $this->height];
        return $this;
    }
    ...
    private function resize_with_crop()
    {
        if (!is_array($this->crop_coordinates)) {
            $this->crop_coordinates = [0, 0, $this->width, $this->height];
        }
    ...
    private function text_dimensions($text, $options)
    {
        if (isset($options["truetype"]) && $options["truetype"]) {
            ...
            return [$text_dimensions[4], $options["size"]];
        } else {
            if ($options["size"] > 5) {
                $options["size"] = 5;
            }
            return ["width" => imagefontwidth($options["size"])*strlen($text),
    ...
    function filter($filter, $ammount = 1, $args = [])
    {
